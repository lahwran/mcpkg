...
class Repo(object):
    ...
    def _load(self):
        ...
        try:
            ...
            raise RepositoryException("Failed to load repository: %s " % e)
        finally:
            f.close()
        ...
        if not self.silent:
            ...
            print "%s section(s) in repository, totalling %s packages" % (len(self.sections), pkgtotal)
            for section in self.sections:
                print "Section '%s' with '%s' packages" % (section.name, len(section.packages))
